...
class TestPaydayCharge(PaydayHarness):
    ...
    def test_payday_moves_money_with_balanced(self):
        ...
        jeff = self.make_participant('jeff2', claimed_time=day_ago,
                                     balanced_customer_href=coinbase_customer.href,
                                     last_bill_result='',
                                     is_suspicious=False)
        ...
        assert bob.balance == D('0.00')
        assert carl.balance == D('2.00')
        assert jeff.balance == D('7.41')
        ...
        jeff_customer = balanced.Customer.fetch(jeff.balanced_customer_href)
